...
- Для темы «Дежурные»: ежедневный неанонимный опрос в `10:00`, закрытие после `00:00`

Чтобы включить опрос дежурных, администратор должен один раз отправить команду
`/setup_duty_poll` прямо внутри нужной темы Telegram. Бот автоматически запомнит
группу и ID темы. Команда `/disable_duty_poll` в той же теме отключает отправку.
